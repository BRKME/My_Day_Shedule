...
# Будни — «Занятия English» (с 22.09.2026; раньше границей было «Читать в
# дороге», которое убрали из утра), суббота — «Включи мозг».
MORNING_BOUNDARIES = ("Занятия English", "Включи мозг")
...
# ── Мудрость дня ─────────────────────────────────────────────────────────
# Раньше фраза была одна, и random.choice выдавал её каждое утро. Теперь
# они чередуются по дате: повторный рендер сообщения не подменит текст.

WISDOMS = (
    "Фокусируешься на решениях — находишь их даже в безвыходных, казалось бы, "
    "ситуациях. Концентрируешься на проблемах — получаешь их в полном объёме "
    "и даже больше.",
    "Мы живём ту жизнь, на которую нам хватило смелости.",
)


def wisdom_of_the_day(day) -> str:
